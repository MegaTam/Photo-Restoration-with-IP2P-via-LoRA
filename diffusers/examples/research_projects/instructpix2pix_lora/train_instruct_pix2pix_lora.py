...
def main():
    ...
    def preprocess_images(examples):
        # 修正: 在这里从文件路径加载图片
        original_images = np.concatenate(
            [convert_to_np(Image.open(os.path.join(args.dataset_name, path)), args.resolution) for path in examples[original_image_column]]
        )
        # 修正: 在这里也从文件路径加载图片
        edited_images = np.concatenate(
            [convert_to_np(Image.open(os.path.join(args.dataset_name, path)), args.resolution) for path in examples[edited_image_column]]
        )
        # We need to ensure that the original and the edited images undergo the same
        # augmentation transforms.
        images = np.concatenate([original_images, edited_images])
